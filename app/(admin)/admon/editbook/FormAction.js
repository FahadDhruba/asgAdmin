...
export default function FormAction({ bookData, bookId }) {
  const [tryReadInputs, setTryReadInputs] = useState(bookData.tryReadSome);
  const [formData, setFormData] = useState({
    bookTitle: bookData.title,
    cover: bookData.cover,
    authors: bookData.author.join(', '),
    price: bookData.price,
    mainPrice: bookData.mainPrice,
    level: bookData.levelId,
    category: bookData.categoryId,
    subject: bookData.subjectId,
    paper: bookData.paperId,
    chapters: bookData.chapter.map(chap => chap.id),
    bookCDNLink: bookData.linka,
  });
  ...
  useEffect(() => {
    setTryReadInputs(bookData.tryReadSome);
    setFormData({
      bookTitle: bookData.title,
      cover: bookData.cover,
      authors: bookData.author.join(', '),
  ...
  return (
    <div className="p-2">
      ...
      <form onSubmit={handleSubmit} className="bg-white px-4 py-2 w-full">
